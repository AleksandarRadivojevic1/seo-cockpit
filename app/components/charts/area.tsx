...
export function Area({
  ...
  // LOCAL PATCH (see line.tsx for the same fix on the line chart).
  //
  // `getY` above returns 0 for a datum with no numeric value, so without a
  // `defined` accessor visx/d3 draw straight through a missing day at the
  // axis -- a false traffic cliff rather than a gap. A null here means "no
  // row for that date"; a real 0 means "measured, and it was zero", and the
  // two must not render alike.
  //
  // Re-apply this after any `shadcn add @bklit/area-chart`: the CLI
  // overwrites this file with upstream's unpatched version.
